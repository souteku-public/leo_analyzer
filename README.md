...
**Starlinkの位置情報について**: `get_status` には緯度経度が含まれません。
Starlinkアプリの 設定 → 詳細設定 で「ローカルネットワークからのアクセス」と
位置情報の共有を有効にすると、`get_location` から緯度・経度・高度を取得して
CSVに記録します(無効の場合は1度だけ警告を表示し、以降は取得を試みません)。
