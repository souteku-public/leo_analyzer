...
- **StarlinkとKymetaを同一画面で比較** — 「比較用に追加…」でもう1つの測定を
  重ねると、同じ指標を同じ色・同じ縦スケールで並べます(A=実線 / B=破線)。
  タイルと平均値もA/B併記になります
...
- 同じ指標(下り・上り・RTT・ロス率など)を**同じ色・同じ縦スケール**で重ねます。
  どちらが速いかを目で比べられるよう、色は指標ごとに固定し、
  **A=実線 / B=破線**で回線を区別します
- タイルと平均値は「180.0 / 118.5 Mbps」のようにA/B併記になります
- 時間軸は**両方の和集合**です。片方だけ長い測定でも切り落としません
- 地図の走行軌跡もA/Bで線種が分かれます(同じ車両に載せた2台なら重なります)
