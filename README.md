...
| `--baseline` | 15 | 無負荷でのRTT測定時間。測定開始前と下り→上りの間に入る(`0` で省略可) |
...
測定は「無負荷15秒 → 下り → 無負荷15秒 → 上り」の順に進み、無負荷区間の
RTTが **アイドルRTT(回線の素の応答時間)** として記録されます。転送中の
RTTは、回線を飽和させたときにバッファに溜まる待ち時間を含むため
**数百ms〜に跳ね上がるのが正常**です(バッファブロート)。これは異常では
なく「飽和時にはこれだけ遅延する」という回線特性です。

summary.json にも分けて集計されます:

```json
"latency_ms": {
  "idle":            { "avg": 75.1, ... },   ← アイドルRTT(この値が回線の実力)
  "download_loaded": { "avg": 210.4, ... },  ← 下り飽和中のRTT
  "upload_loaded":   { "avg": 540.2, ... }   ← 上り飽和中のRTT
}
```

目安: アイドルRTTが Starlink で 20〜60ms、OneWeb で 50〜150ms 程度なら
正常です。アイドルRTTが500msを超えるような場合は経路異常(遠回りの
ゲートウェイ/PoP、VPN経由など)を疑ってください。
