...
- **15秒ハンドオーバー境界** — Starlinkの接続組み替えタイミングを
  タイムラインに重ね、損失がそこに乗ったかを色分け
...
Starlink はUTCの15秒境界(:00 / :15 / :30 / :45)で接続を組み替えます。
ビューアはこの境界を縦線で表示し、パケット損失がそこに乗ったかどうかで
色分けします(赤=境界に乗った損失、灰=それ以外)。集計値も
「損失◯秒のうち◯秒(◯%)が境界直後2秒」という形で表示されるので、
**損失がハンドオーバー起因かどうかを判断**できます。均一なら13%です。
